...
def get_shots_from_inner_html(inner_html, hole):
    shots = {}
    ...
    for stuff in re.compile("marker(?: plus| hidden)? shot").split(inner_html):
        ...
        if r:
            shots[shot].x = int(MAP_WIDTH * (int(r.group(1)) + SHOT_MARKER_RADIUS) / 681)
            shots[shot].y = int(MAP_HEIGHT * (int(r.group(2)) + SHOT_MARKER_RADIUS) / 511)
